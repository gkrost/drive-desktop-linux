  ...
  return new Proxy({} as any, {
    get: (target: any, prop: string | symbol) => {
      ...
      if (!target[prop]) {
        ...
        // Nested objects (like devices, cleaner) should be Proxies with mock function properties
        target[prop] = vi.fn();

        // Add a Proxy to support nested property access (e.g., devices.getDevices)
