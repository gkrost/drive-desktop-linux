...
const OpcodesAndDefaults = new Map([
    ["init", { op: binding.op_init }],
    ["error", { op: binding.op_error }],
    ["access", { op: binding.op_access, defaults: [0] }],
    ["statfs", { op: binding.op_statfs, defaults: [getStatfsArray()] }],
    ["fgetattr", { op: binding.op_fgetattr, defaults: [getStatArray()] }],
    ["getattr", { op: binding.op_getattr, defaults: [getStatArray()] }],
    ["flush", { op: binding.op_flush }],
    ["fsync", { op: binding.op_fsync }],
    ["fsyncdir", { op: binding.op_fsyncdir }],
    ["readdir", { op: binding.op_readdir, defaults: [[], []] }],
    ["truncate", { op: binding.op_truncate }],
    ["ftruncate", { op: binding.op_ftruncate }],
    ["utimens", { op: binding.op_utimens }],
    ["readlink", { op: binding.op_readlink, defaults: [""] }],
    ["chown", { op: binding.op_chown }],
    ["chmod", { op: binding.op_chmod }],
    ["mknod", { op: binding.op_mknod }],
    ["setxattr", { op: binding.op_setxattr }],
    ["getxattr", { op: binding.op_getxattr }],
    ["listxattr", { op: binding.op_listxattr }],
    ["removexattr", { op: binding.op_removexattr }],
    ["open", { op: binding.op_open, defaults: [0] }],
    ["opendir", { op: binding.op_opendir, defaults: [0] }],
    ["read", { op: binding.op_read, defaults: [0] }],
    ["write", { op: binding.op_write, defaults: [0] }],
    ["release", { op: binding.op_release }],
    ["releasedir", { op: binding.op_releasedir }],
    ["create", { op: binding.op_create, defaults: [0] }],
    ["unlink", { op: binding.op_unlink }],
    ["rename", { op: binding.op_rename }],
    ["link", { op: binding.op_link }],
    ["symlink", { op: binding.op_symlink }],
    ["mkdir", { op: binding.op_mkdir }],
    ["rmdir", { op: binding.op_rmdir }],
]);
...
class Fuse extends Nanoresource {
    constructor(mnt, ops, opts = {}) {
        ...
        /**
         * As mentioned in the discussion below, `path.resolve()` mangles the
         * mountpoint when given as a drive letter on Windows, e.g. "X:", into
    ...
    _makeHandlerArray() {
        ...
        function makeHandler(name, op, defaults, nativeSignal) {
            ...
            function signal(nativeHandler, error, ...args) {
                var arr = [nativeHandler, error];
                
                // Only include buffer in args if it's a valid buffer (not null/undefined when it should be)
                // For xattr operations returning error codes without data, don't pass the buffer
            ...
            function autoTimeout(cb, input) {
                ...
                function timeoutWrap(error, ...args) {
                    if (called) {
                        return;
                    }
                    called = true;
    ...
    _open(cb) {
        ...
        if (this._force) {
            return fs.stat(path.join(this.mnt, "test"), (error, st) => {
                if (error && (error.errno === ENOTCONN || error.errno === Fuse.ENXIO)) {
    ...
    _close(cb) {
        ...
        Fuse.unmount(this.mnt, (error) => {
            if (error) {
                error.unmountFailure = true;
                return cb(error);
            }
            nativeUnmount();
        });
    ...
    _op_init(signal) {
        ...
        this.ops.init((error) => signal(error));
    }
    ...
    _op_error(signal) {
        ...
        this.ops.error((error) => signal(error));
    }

    _op_statfs(signal, path) {
        this.ops.statfs(path, (error, statfs) => {
            if (error) {
                return signal(error);
            }
    ...
    _op_getattr(signal, path) {
        ...
        this.ops.getattr(path, (error, stat) => {
            if (error) {
                return signal(error, getStatArray());
            }
    ...
    _op_access(signal, path, mode) {
        this.ops.access(path, mode, (error) => signal(error));
    }
    ...
    _op_utimens(signal, path, atimeLow, atimeHigh, mtimeLow, mtimeHigh) {
        ...
        this.ops.utimens(path, atime, mtime, (error) => signal(error));
    }

    _op_release(signal, path, fd) {
        this.ops.release(path, fd, (error) => signal(error));
    }

    _op_releasedir(signal, path, fd) {
        this.ops.releasedir(path, fd, (error) => signal(error));
    }

    _op_read(signal, path, fd, buf, len, offsetLow, offsetHigh) {
        this.ops.read(path, fd, buf, len, getDoubleArg(offsetLow, offsetHigh), (error, bytesRead) => {
            return signal(error, bytesRead || 0, buf.buffer);
        });
    }

    _op_write(signal, path, fd, buf, len, offsetLow, offsetHigh) {
        this.ops.write(path, fd, buf, len, getDoubleArg(offsetLow, offsetHigh), (error, bytesWritten) => {
            return signal(error, bytesWritten || 0, buf.buffer);
        });
    }

    _op_readdir(signal, path) {
        this.ops.readdir(path, (error, names, stats) => {
            if (error) {
                return signal(error);
            }
    ...
    _op_setxattr(signal, path, name, value, position, flags) {
        this.ops.setxattr(path, name, value, position, flags, (error) => {
            return signal(error, value.buffer);
        });
    }

    _op_getxattr(signal, path, name, valueBuf, position) {
        this.ops.getxattr(path, name, position, (error, value) => {
            if (!error) {
                if (!value) {
                    // Return error code with null buffer to prevent native assertion failure
                    return signal(IS_OSX ? -93 : -61, null);
                }
                value.copy(valueBuf);
                return signal(value.length, valueBuf.buffer);
            }
            // Return error code with null buffer
            const errorCode = typeof error === 'number' ? error : 0;
            return signal(errorCode, null);
        });
    }
                value.copy(valueBuf);
                return signal(value.length, valueBuf.buffer);
            }
            // Return error code without buffer
            const errorCode = typeof error === 'number' ? error : 0;
            return process.nextTick(binding.fuse_native_signal_getxattr, signal(true).nativeHandler, errorCode);
        });
    }

    _op_listxattr(signal, path, listBuf) {
        this.ops.listxattr(path, (error, list) => {
            if (list && !error) {
                if (!listBuf.length) {
                    let size = 0;
                    for (const name of list) size += Buffer.byteLength(name) + 1;
                    size += 128; // fuse yells if we do not signal room for some mac stuff also
                    return signal(size, null);
                }
    ...
    _op_removexattr(signal, path, name) {
        this.ops.removexattr(path, name, (error) => signal(error));
    }

    _op_flush(signal, path, fd) {
        this.ops.flush(path, fd, (error) => signal(error));
    }

    _op_fsync(signal, path, datasync, fd) {
        this.ops.fsync(path, datasync, fd, (error) => signal(error));
    }

    _op_fsyncdir(signal, path, datasync, fd) {
        this.ops.fsyncdir(path, datasync, fd, (error) => signal(error));
    }

    _op_truncate(signal, path, sizeLow, sizeHigh) {
        const size = getDoubleArg(sizeLow, sizeHigh);
        this.ops.truncate(path, size, (error) => signal(error));
    }

    _op_ftruncate(signal, path, fd, sizeLow, sizeHigh) {
        const size = getDoubleArg(sizeLow, sizeHigh);
        this.ops.ftruncate(path, fd, size, (error) => signal(error));
    }

    _op_readlink(signal, path) {
        this.ops.readlink(path, (error, linkname) => {
            return signal(error, linkname);
        });
    }

    _op_chown(signal, path, uid, gid) {
        this.ops.chown(path, uid, gid, (error) => signal(error));
    }

    _op_chmod(signal, path, mode) {
        this.ops.chmod(path, mode, (error) => signal(error));
    }

    _op_mknod(signal, path, mode, dev) {
        this.ops.mknod(path, mode, dev, (error) => signal(error));
    }

    _op_unlink(signal, path) {
        this.ops.unlink(path, (error) => signal(error));
    }

    _op_rename(signal, src, dest) {
        this.ops.rename(src, dest, (error) => signal(error));
    }

    _op_link(signal, src, dest) {
        this.ops.link(src, dest, (error) => signal(error));
    }

    _op_symlink(signal, src, dest) {
        this.ops.symlink(src, dest, (error) => signal(error));
    }

    _op_mkdir(signal, path, mode) {
        this.ops.mkdir(path, mode, (error) => signal(error));
    }

    _op_rmdir(signal, path) {
        this.ops.rmdir(path, (error) => signal(error));
    }

    // Public API

    mount(cb) {
        return this.open((error) => setTimeout(cb, 100, error));
    }
