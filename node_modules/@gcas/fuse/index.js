    ...
    _makeHandlerArray() {
        ...
        function makeHandler(name, op, defaults, nativeSignal) {
            ...
            function signal(nativeHandler, error, ...args) {
                // When error is non-zero (error condition), only pass error code and nativeHandler
                // This prevents native assertion failures when xattr returns errors
                if (error !== 0 && error !== null && error !== undefined) {
                    return process.nextTick(nativeSignal, nativeHandler, error);
                }
                
                var arr = [nativeHandler, error];
                
                // Only include additional args (like buffer) if they are valid
