...
// the variables are injected via webpack in prod
...
import { logPreStartupConfig, logPostStartupConfig } from './startup-logger';

logger.debug({ msg: '[DEEPLINK] process.argv at launch:', argv: process.argv });

const gotTheLock = app.requestSingleInstanceLock();
logger.debug({ msg: '[DEEPLINK] Single instance lock acquired:', gotTheLock });

const protoResult = app.setAsDefaultProtocolClient('internxt');
logger.debug({ msg: '[DEEPLINK] setAsDefaultProtocolClient result:', protoResult });

if (!gotTheLock) {
  logger.debug({ msg: '[DEEPLINK] Another instance is running, quitting.' });
  app.quit();
  // app.quit() doesn't stop JS execution — we must exit immediately
  // to prevent the second instance from registering IPC handlers,
  // creating windows, or loading localhost:1212 (which fails).
  process.exit(0);
}
...
app.on('second-instance', async (_, argv) => {
  logger.debug({ tag: 'AUTH', msg: '[DEEPLINK] second-instance event fired', argv });
  const deeplinkArg = argv.find((arg) => arg.startsWith('internxt://'));
  if (!deeplinkArg) {
    logger.debug({ tag: 'AUTH', msg: '[DEEPLINK] No internxt:// URL found in argv, ignoring.' });
    return;
  }
  logger.debug({ tag: 'AUTH', msg: '[DEEPLINK] Processing URL:', url: deeplinkArg });
...
eventBus.on('USER_LOGGED_IN', async () => {
  try {
    ...
    logPostStartupConfig(configStore);
  } catch (error) {
    logger.error({
